    ...
    def handle_connected(self):
        ...
        for x in col.index:
            ...
            self.oids[x]["bid"] = self.place_order(
                OrderType.LIMIT, OrderSide.BID, 400000000, x, "%.2f" % bid)[1].order_id
            self.oids[x]["ask"] = self.place_order(
                OrderType.LIMIT, OrderSide.ASK, 400000000, x, "%.2f" % ask)[1].order_id
            ...
            for x in col.index:
                ...
                if val < self.oids[x]["mid_px"]:
                    self.oids[x]["bid"] = self.modify_order(
                        self.oids[x]["bid"], OrderType.LIMIT, OrderSide.BID, 400000000, x, "%.2f" % bid)[1].order_id
                    self.oids[x]["ask"] = self.modify_order(
                        self.oids[x]["ask"], OrderType.LIMIT, OrderSide.ASK, 400000000, x, "%.2f" % ask)[1].order_id
                else:
                    self.oids[x]["ask"] = self.modify_order(
                        self.oids[x]["ask"], OrderType.LIMIT, OrderSide.ASK, 400000000, x, "%.2f" % ask)[1].order_id
                    self.oids[x]["bid"] = self.modify_order(
