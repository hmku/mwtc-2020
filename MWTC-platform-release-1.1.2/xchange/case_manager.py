...
class CaseManager(RoundManager):
    ...
    def num_connected_bots(self) -> int:
        """
        Get the number of competitors connected to the exchange

        Returns:
            (int) The number of competitors connected to the exchange (0 if not running)
        """

        resp = self._rm_stub.NumConnectedBotsUpdate(round_manager.NumConnectedBotsRequest(
                competitor=self.creds
        ))
